...
exports.submitAssignment = async (req, res, next) => {
  try {
    ...
    await newSubmission.populate("studentId", "fullName email userId phone");
...
exports.getMySubmissions = async (req, res, next) => {
  try {
    ...
    const submissions = await Submission.find({ studentId })
      .populate("assignmentId", "title description deadline maxScore batchId")
...
exports.getAllSubmissions = async (req, res, next) => {
  try {
    const submissions = await Submission.find()
      .populate("studentId", "fullName email userId phone")
      .populate("assignmentId", "title maxScore batchId")
      .populate("gradedBy", "fullName email")
      .sort({ createdAt: -1 });
