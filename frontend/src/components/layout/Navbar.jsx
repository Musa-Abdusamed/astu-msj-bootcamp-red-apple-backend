...
export default function Navbar({ onOpenLogin, onOpenApply }) {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-40 transition-all duration-300 ${
      isScrolled ? 'bg-white/95 backdrop-blur-md shadow-sm border-b border-slate-100 py-3.5' : 'bg-transparent py-5'
    }`}>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-3">
            <button
              ...
              className="px-5 py-2 rounded-xl text-sm font-semibold text-indigo-600 border border-indigo-200 hover:bg-indigo-50 hover:border-indigo-300 transition-all duration-200 active:scale-95"
            >
              Log In
            </button>
            <button
              onClick={() => (onOpenApply ? onOpenApply() : null)}
              className="px-5 py-2 rounded-xl text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 shadow-sm shadow-indigo-300/40 transition-all duration-200 active:scale-95 hover:shadow"
            >
              Apply Now
            </button>
          </div>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-white/98 backdrop-blur-lg border-b border-slate-200 px-4 pt-3 pb-6 space-y-3 animate-fadeIn">
          ...
          <div className="pt-4 border-t border-slate-100 flex flex-col gap-2.5">
            <button
              onClick={() => { setMobileMenuOpen(false); onOpenLogin && onOpenLogin(); }}
              className="w-full py-2.5 rounded-xl text-sm font-semibold text-indigo-600 border border-indigo-200 hover:bg-indigo-50 text-center"
            >
              Log In
            </button>
            <button
              onClick={() => { setMobileMenuOpen(false); onOpenApply && onOpenApply(); }}
              className="w-full py-2.5 rounded-xl text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 text-center shadow-sm shadow-indigo-300"
            >
              Apply Now
            </button>
          </div>
